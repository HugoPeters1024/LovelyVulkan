#pragma once
#include "precomp.h"
#include "AppContext.h"
#include "AppExt.h"

namespace lv {

struct RasterizerAttachment {
    uint32_t binding;
    std::function<VkImageView(FrameContext& frame)> imageSelector;
};

struct RasterizerTexture {
    uint32_t binding;
    std::function<VkImageView(FrameContext& frame)> imageSelector;
};

struct RasterizerInfo {
    RasterizerInfo(std::string vertShaderPath, std::string fragShaderPath)
        : vertShaderPath(vertShaderPath), fragShaderPath(fragShaderPath) {}

    void defineAttachment(uint32_t slot, std::function<VkImageView(FrameContext& frame)> imageSelector) {
        attachments.push_back(RasterizerAttachment {slot, imageSelector});
    }

    void defineTexture(uint32_t slot, std::function<VkImageView(FrameContext& frame)> imageSelector) {
        textures.push_back(RasterizerTexture {slot, imageSelector});
    }


    std::string vertShaderPath;
    std::string fragShaderPath;
    std::vector<RasterizerAttachment> attachments;
    std::vector<RasterizerTexture> textures;
};

struct RasterizerFrame {
    VkDescriptorSet descriptorSet;
    VkFramebuffer framebuffer;
    VkSampler sampler;
};

class Rasterizer : public AppExt<RasterizerFrame> {
public:
    Rasterizer(AppContext& ctx, RasterizerInfo info);
    ~Rasterizer();

    RasterizerFrame* buildFrame(FrameContext& frame) override;
    void destroyFrame(AppContext& ctx, RasterizerFrame* frame) override;

    void startPass(FrameContext& frame) const;
    void endPass(FrameContext& frame) const;

private:
    void createRenderPass();
    void createDescriptorSetLayout();
    void createPipelineLayout();
    void createPipeline();



//------------- FIELDS -------------------

private:
    RasterizerInfo info;
    VkRenderPass renderPass;
    VkPipelineLayout pipelineLayout;
    VkPipeline pipeline;
    VkDescriptorSetLayout descriptorSetLayout;
};

}
